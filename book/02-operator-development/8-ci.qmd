...
- **Private images are fine.** Tercen can pull private operator images; the image does not need to be public to be installable. The pull authenticates with the **installer's own** credential — a classic PAT with `read:packages` (SSO-authorize for the org), attached at install and cleared when the install task finishes; Tercen never provisions registry credentials. See [Installing an Operator](9-installation.qmd).
...
**Fix:** pass a **classic Personal Access Token with `repo` scope** instead of the built-in token. **A fine-grained token (`github_pat_...`) will NOT work** — even with Contents: Read correctly set, the `github.com/<org>/<repo>/zipball/<ref>` endpoint the Tercen server calls only accepts classic (`ghp_`) and OAuth tokens; a fine-grained token yields the same 404. (Verified 2026-08: a fine-grained token that fetched the same zipball successfully via `api.github.com` was rejected on the `github.com` URL.) Store the classic PAT as an **organization secret named `TERCEN_GITHUB_TOKEN`** so all operator repos share it:
      ...
      - name: Install operator
        run: |
          docker run --rm \
            ...
            -e GITHUB_TOKEN=${{ secrets.TERCEN_GITHUB_TOKEN }} \   # CLASSIC repo-scoped PAT (org secret), NOT the built-in GITHUB_TOKEN, NOT a fine-grained token
...
This only affects the optional CI **install** step. The `build-and-push` of the versioned image runs *before* it and succeeds regardless — so the operator image exists and can be installed **via the library** even if this step is red. Prefer keeping the Install step **blocking**: a release whose operator cannot be installed and tested is not a release you want published (do not use `continue-on-error` to silence it — that ships unverified releases and hides real breakage, as happened with `uka_operator`, broken silently for four months). If a repo genuinely has no install-test yet, fix that by adding a `tests/test.json` rather than by removing the check. (Reminder from earlier: the repo name must end in `_operator` for the library add to install it.)
:::

## The install-check is fatal by design

A release whose operator cannot install and pass its unit test must not be
published — keep the Install step **blocking**. Do not silence it with
`continue-on-error` or `if: false`: that shipped `uka_operator` broken for
four months with nobody noticing. A red release burns a patch number; fix the
cause (missing `tests/test.json`, wrong token, golden output out of date —
see the operator-improvements chapter) and tag the next number. Burned
version numbers are normal and free.
