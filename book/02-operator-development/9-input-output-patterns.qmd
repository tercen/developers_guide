...
For operators that generate files (plots, reports, models):
...
# Create file table for Tercen
file_table <- data.frame(
  filename = basename(plot_filename),
  mimetype = "image/png"
) %>%
  ctx$addNamespace()
...
df = tercenCtx.select(['.ci', '.y'], df_lib="pandas")

plt.figure(figsize=(8, 6))
sns.scatterplot(data=df, x='.ci', y='.y')
...
file_table = tercenCtx.add_namespace(file_table)
tercenCtx.save(file_table)
```
