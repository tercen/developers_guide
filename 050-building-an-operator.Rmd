...
Here we will learn through a general approach to create an R operator for Tercen. A more detailed and concreate example is outlined in the next chapter where a linear regression operator is built from scratch, in the form of a tutorial.
...
The concept of an input project and an output relation helps in defining your operator.



The __input project__ is the project in the data step which is used to use the operator. For example what is on the `y-axis`, `x-axis`, `row`, `col`, `color`, `label`. This input project determines what is the structure of the data which is given to the operator.
...
__Per Cell__ example : Let's say the operator used a projection with `row` and `col` and the operator calculates a mean per row/col (i.e. per cell), this would be a per cell relationship. Excel, for example, is always calculating per cell relationships.
...
__for All__ data example: Let's say the operator used a project with `row` and `col` and the operator calculated a total mean of all the data, this would be an all data relationship.
...
* Now that the repository is initialized in your GitHub account, you can work on it locally, using tercen-studio for example.

## Setup the input data {-}

This can be in the local or a cloud instance of Tercen.
